...
void threshold_sliced(void *args) {
  ...
  for (long i = start_pixel_idx; i < end_pixel_idx; i += channels) {
    int gray_value =
        (channels == 1) ? src[i] : (src[i] + src[i + 1] + src[i + 2]) / 3;

    if (gray_value <= args_thr.slice_t1_thr ||
        gray_value >= args_thr.slice_t2_thr) {
      dest[i] = 255;
      if (channels > 1) {
        dest[i + 1] = 255;
        dest[i + 2] = 255;
...
void process_img_thread(void *args) {
  ...
  for (int i = 0; i < NTASKS; i++) {
    ...
    slice_tasks[i].end_row =
        (i == NTASKS - 1) ? img->height : (i + 1) * rows_per_thread;
  ...
  stbi_write_png(output_filename, img->width, img->height, img->channels,
                 output_pixels, img->width * img->channels);
...
int main() {
  ...
  while (1) {
    ...
    printf(
        "Recebido imagem: %dx%d, %d canais, %d bytes (pos=%d) para task: %d\n",
